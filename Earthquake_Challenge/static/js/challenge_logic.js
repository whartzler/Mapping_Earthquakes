...
// Adding layers to the navigation pane
let allEarthquakes = new L.LayerGroup();
let tectonicPlates = new L.LayerGroup();
let majorEarthquakes = new L.LayerGroup();


//Add a references to each overlays object.
let overlays = {
...
var majorEarthquakesData = 'https://earthquake.usgs.gov/earthquakes/feed/v1.0/summary/4.5_week.geojson'

d3.json(majorEarthquakesData).then(function(data) {
  
  //styling the major earthquakes features
  function styleInfo(feature) {
    return {
      opacity: 1,
      fillOpacity: 1,
  ...
  // Adding geoJSOn layer to the map and describing each major earthquake and features
  L.geoJSON(data, {
    pointToLayer: function(feature, latlng) {
      return L.circleMarker(latlng);
    },
...
//Add major earthquakes to the map
...
let tectonicPlateData = "https://raw.githubusercontent.com/fraxen/tectonicplates/master/GeoJSON/PB2002_boundaries.json"
  ...
  d3.json(tectonicPlateData).then(function (data) {
    ...
    // Addting Tectonic Plate data into geoJSON layer
    L.geoJSON(data, {
      color: 'red',
      weight: 2,
    }).addTo(tectonicPlates)

    //adding the style to the map
